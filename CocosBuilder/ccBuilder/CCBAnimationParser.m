...
+ (NSMutableArray*) findAnimationsAtPath:(NSString*)assetsPath
{
	...
	FindValidateBlock validateBlock = ^BOOL ( NSString* fileName, NSURL* url )
	{
		NSString* pathExt = [fileName pathExtension];
		if ([pathExt isEqualToString:@"plist"] && [CCBAnimationParser isAnimationFileURL:url])
			return true; 
		return false;
	};
	...
	FindSuccessBlock successBlock = ^( NSString* relativePath, NSString* absPath )
	{
		NSLog(@"file is animation: %@", absPath);
		
		[assets addObject:relativePath];
		[[CCAnimationCache sharedAnimationCache] addAnimationsWithFile:absPath];
	};

	[CCBUtil findFilesInDirectory:assetsPath validationBlock:validateBlock successBlock:successBlock];
