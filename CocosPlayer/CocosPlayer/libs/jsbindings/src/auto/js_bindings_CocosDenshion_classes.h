/*
* AUTOGENERATED FILE. DO NOT EDIT IT
* Generated by "generate_js_bindings.py -c CocosDenshion_jsb.ini" on 2012-10-18
* Script version: v0.3
*/
#import "js_bindings_config.h"
#ifdef JSB_INCLUDE_COCOSDENSHION

#import "js_bindings_NS_manual.h"

/*
 * SimpleAudioEngine
 */
#pragma mark - SimpleAudioEngine


#ifdef __cplusplus
extern "C" {
#endif

void JSB_SimpleAudioEngine_createClass(JSContext *cx, JSObject* globalObj, const char* name );



extern JSObject *JSB_SimpleAudioEngine_object;
extern JSClass *JSB_SimpleAudioEngine_class;

#ifdef __cplusplus
}
#endif


/* Proxy class */
@interface JSB_SimpleAudioEngine : JSB_NSObject
{
}

@end


#endif // JSB_INCLUDE_COCOSDENSHION
